...
router.get("/fetch_strategies/:page/:date", async(req, res) => {
    ...
    const strategies = await Strategy.find({type:type, user_id:ObjectId(user_id), active:true}).sort({'picks_sent': -1}).skip((page - 1) * 5).limit(5);
...
router.post("/fixtures", async(req, res) => {
    ...
    const strategy = await Strategy.findOne({ _id: strategy_id });
    const fixtures = await Fixture.findUpcomingResults(
        strategy,
        "upcoming",
        date,
